...
int allocate_frame(pgtbl_entry_t *p) {
    ...
    if(frame == -1) { // Didn't find a free page.
        ...
        pgtbl_entry_t *victim = coremap[frame].pte; // Saving the victim frame
        int temp_off;
        if (victim->frame & PG_DIRTY) {
            temp_off = swap_pageout(frame, victim->swap_off); // Swap the victim frame out of memory 
            victim->swap_off = temp_off;
            evict_dirty_count++; 
        } else {
            evict_clean_count++; 
        }
        victim->frame &= ~PG_VALID; // Page is no longer in memory
        victim->frame &= ~PG_DIRTY; // Page is no longer in memory
        victim->frame |= PG_ONSWAP; // Page has been swapped out
	}
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    // Initialize the second level if pgdir has not been initialized
    if (!(pgdir[idx].pde)) {
    ...
    p = &pageTable[idx]; // Initialize p to point to a page table entry in the page tables

    // Check if p is valid or not, on swap or not, and handle appropriately
    if (p->frame & PG_VALID) {
        hit_count++; // Hit when page is in memory
    } else {
        miss_count++; // Miss when page is not in memory
        int current = allocate_frame(p); // Store the PFN
        coremap[current].vaddr = vaddr; // Allows opt to function
        if (p->frame & PG_ONSWAP) { // Page is in swap space
            swap_pagein(current, p->swap_off); // Swap in page
            p->frame = current << PAGE_SHIFT; // Get the PFN
            p->frame &= ~PG_ONSWAP; // Page is now in memory and not in swap
            p->frame &= ~PG_DIRTY;
        } else { // Page is not in swap space but is uninitialized
            init_frame(current, vaddr); // Initialize the frame
            p->frame = current << PAGE_SHIFT; // Get the PFN
            p->frame |= PG_DIRTY; // Page has been modified so it's dirty now
        }
    }
