...
MODULE_AUTHOR("Andrew Te & Joseph Leeroy");
MODULE_LICENSE("GPL");
...
void my_exit_group(int status) {
    // Lock the pidlist, ENTERING CRITICAL SECTION
    spin_lock(&pidlist_lock); 
    del_pid(current->pid); // Delete the pid from all lists of pids being monitored
    spin_unlock(&pidlist_lock);
    
    orig_exit_group(status); // Calling the original exit group    
}
//----------------------------------------------------------------
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    //  Log the message if monitored == 2 or pid is monitored
    if (table[reg.ax].monitored == 2 || 
		(table[reg.ax].monitored == 1 && check_pid_monitored(reg.ax, current->pid))) {
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}
	
	return table[reg.ax].f(reg); // Return the syscall
}
 ...
 *     d) Cannot stop monitoring a single pid while monitoring all pids using pid=0.
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    	...
    	// The input is invalid when the syscall is negative, root syscall or the same as MY_CUSTOM_SYSCALL
	if (syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL)
                return -EINVAL;
        
	// Invalid permissions when not in root
        if ((cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) && current_uid())
                return -EPERM;
	
        if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {
                
                // Invalid input whenever the pid specified is negative and does not exist
                if (pid < 0 && (pid != 0 && !pid_task(find_vpid(pid), PIDTYPE_PID)))
                        return -EINVAL;

                // Invalid permisions if not root or pid is not specified
                if (current_uid() && (check_pid_from_list(current->pid, pid) || !pid))
                        return -EPERM;
        }
        ...
        if (cmd == REQUEST_SYSCALL_INTERCEPT) {
                ...
                // Cannot intercept an already intercepted syscall
                if (table[syscall].intercepted == 1)
                        return -EBUSY;
    
		spin_lock (&pidlist_lock); // LOCK PIDLIST, GOING IN CRITICAL SECTION
		table[syscall].f = sys_call_table[syscall]; // Saving syscall to table[]
		table[syscall].monitored = (pid==0) ? 2 : 1; // Set monitored to 2 when pid == 0, otherwise will set it to 1
		table[syscall].intercepted = 1; // Process is now intecepted
		
        	// When you cannot add a syscall to the pidlist, will return -ENOMEM
        	if (add_pid_sysc(pid, syscall)) {
			spin_unlock (&pidlist_lock);
			return -ENOMEM;
		}
		spin_unlock (&pidlist_lock); // LEAVING CRITICAL SECTION

                spin_lock (&calltable_lock); // ENTERING CRITICL SECITON
                set_addr_rw((unsigned long) sys_call_table); // Set syscall table to be writeable
                sys_call_table[syscall] = &interceptor; // Hijacking syscall with intercptor function
                set_addr_ro((unsigned long) sys_call_table); // Set syscall table to read only
                spin_unlock (&calltable_lock);  // LEAVING THE CRITICAL SECTION
        }

        // Syscall Release Command
        if (cmd == REQUEST_SYSCALL_RELEASE) {
                // Input is invalid if the syscall wasn't originally intercepted
                if (table[syscall].intercepted == 0)
                        return -EINVAL;

		spin_lock (&calltable_lock);  // ENTERING CRITICAL SECITON
		set_addr_rw((unsigned long) sys_call_table); // Set syscall table to be writeable
		sys_call_table[syscall] = table[syscall].f; // Unhijacking the syscall
		set_addr_ro((unsigned long) sys_call_table); // Set syscall table to be read only
		spin_unlock (&calltable_lock); // LEAVING CRITICAL SECITON
		
		spin_lock (&pidlist_lock);  // ENTERING CRITICAL SECITON
		table[syscall].intercepted = 0;  // Syscall is no longer intercepted
		spin_unlock (&pidlist_lock);  // LEAVING CRITICAL SECITON
        }


        //  Syscall Monitor for PID Command
        if (cmd == REQUEST_START_MONITORING) {		
        	// Cannot add PID to list if everything is monitored or PID already exists in the list
        	if (table[syscall].monitored == 2 || check_pid_monitored(syscall, pid))
			return -EBUSY;
		
		spin_lock (&pidlist_lock);  // ENTERING CRITICAL SECITON
		if (pid==0) table[syscall].monitored = 2; // When pid == 0, everything is monitored = 2
		else if (add_pid_sysc(pid, syscall)) { 
			spin_unlock (&pidlist_lock);	
			return -ENOMEM;  // There is no memory left if add_pid_syscall returns anything but a zero
		}
		table[syscall].monitored = 1;
		spin_unlock (&pidlist_lock);  // LEAVING CRITICAL SECTION
        }

        // Stop Monitoring a Syscall
        if (cmd == REQUEST_STOP_MONITORING) {
        	/* Invalid input when everything is monitored and you want to monitor another PID
         	* or the syscall is not monitored or not intecepted
         	*/
        	if ((table[syscall].monitored == 2 && pid != 0) 
			|| !table[syscall].monitored || !table[syscall].intercepted)
			return -EINVAL;

		spin_lock (&pidlist_lock); // ENTERING CRITICAL SECTION
		if (pid == 0) destroy_list(syscall); // Don't monitor anything when pid == 0
		else if (del_pid_sysc(pid, syscall)) {
			spin_unlock (&pidlist_lock);
			return -EINVAL;  // Invalid input when removing a PID that was not in the list of PIDs
		}
		spin_unlock (&pidlist_lock);  // LEAVING CRITICAL SECTIONS
        }
	return 0;
}
...
static int init_function(void) {
    	...
    	// Lock the calltable, ENTERING CRITICAL SECTION
    	spin_lock (&calltable_lock);     
    	orig_custom_syscall = sys_call_table [MY_CUSTOM_SYSCALL]; // Saving MY_CUSTOM_SYSCALL to orig_custom_syscall
    	orig_exit_group = sys_call_table [__NR_exit_group]; // Saving __NR_exit_group to orig_exit_group
 
	set_addr_rw ((unsigned long) sys_call_table); // Set the syscall table to be read writeable to tamper with
	sys_call_table [MY_CUSTOM_SYSCALL] = &my_syscall; // Discretely hijacking MY_CUSTOM_SYSCALL
	sys_call_table [__NR_exit_group] = &my_exit_group; // Nonchalantly deflowering  __NR_exit_group
	set_addr_ro ((unsigned long) sys_call_table); // Set syscall table to read only like nothing happened to it
	// Unlocking calltable, LEAVING CRITICAL SECTION
    	spin_unlock (&calltable_lock);
    	// Loop through all the non-root system calls to anaLyze them 
   	for (i= 0; i<NR_syscalls;i++) {
        	INIT_LIST_HEAD (&table[i].my_list); // Populate list
        	table[i].intercepted = 0;// By default, syscall isn't being intercepted
        	table[i].monitored = 0; // By default, no PIDs are monitored for the syscall
        	table[i].listcount = 0;// By default, no PIDs want this syscall because so ugly
    	}
...
{   
    	...
    	set_addr_rw ((unsigned long) sys_call_table); // Set the syscall table to be read writeable to tamper with
    	sys_call_table [MY_CUSTOM_SYSCALL] = orig_custom_syscall; // Saving MY_CUSTOM_SYSCALL from orig_custom_syscall
    	sys_call_table [__NR_exit_group] = orig_exit_group; // Saving __NR_exit_group from orig_exit_group
    	set_addr_ro ((unsigned long) sys_call_table); // Set syscall table to read only like nothing happened to it

    	spin_unlock (&calltable_lock); 
    	// Unlocking the calltable, LEAVING THE CRITICAL SECTION

    	//  Loops through all the non-root syscall to not-kill them from list
    	for (i=0;i < NR_syscalls; i++) {
        	destroy_list(i); // Did I say not-kill? I meant kill them all
    	} // Sorry pid list but I needed more memory to play Tank Dating Sims
}
